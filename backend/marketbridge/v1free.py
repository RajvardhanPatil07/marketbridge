...
def _war_room_risk_request(
    ...
    mark = {
        ...
        "POISONED_MARK": (
            base_reference * Decimal("1.03078")
...
def _story(
    ...
    scenario_copy = {
        "NORMAL": {
            ...
            "detail": (
                "MarketBridge can permit new exposure subject to account "
                "and leverage policy."
            ),
        },
        "POISONED_MARK": {
            "headline": (
                "Venue mark diverges sharply from independent Market Truth."
            ),
            "detail": (
                "New exposure fails closed while a valid reduce/close path "
                "remains available."
            ),
        },
        "RECOVERY": {
            "headline": (
                "Evidence has converged again, but recovery is deliberately sticky."
            ),
            "detail": (
                "Hysteresis prevents one good tick from instantly re-enabling "
                "leverage after an incident."
            ),
        },
    }[payload.scenario]
    return {
...
def install_v1_free(
    ...
    if getattr(
        app.state,
        "marketbridge_v1_free_installed",
        False,
    ):
        return
    ...
    def intelligence(
        ...
        if normalized not in {
            ...
            raise HTTPException(
                status_code=404,
                detail="Unsupported intelligence symbol",
            )
        try:
    ...
    def proof_benchmark(
        cases_per_class: int = 50,
    ):
        bounded = min(
            max(cases_per_class, 10),
            200,
        )
        return risk_gate_benchmark(
            root,
            bounded,
        )
    ...
    def war_room_replay(
        payload: WarRoomReplayRequest,
    ):
        try:
            return risk_gateway.replay(
                payload.passport_id,
                payload.mode,
            )
