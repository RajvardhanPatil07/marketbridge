...
class RiskGateway:
    ...
    def market_truth(
        ...
        venue_age = max(
            ...
            (
                now
                - request.market.event_time.astimezone(timezone.utc)
            ).total_seconds(),
        )
        if request.demo_scenario:
            ...
            if request.demo_scenario == "POISONED_MARK":
                raw_state = "HALTED"
            stable = (
                request.demo_scenario in {"NORMAL", "RECOVERY"}
                and divergence < Decimal("75")
            )
            recovery = self.recovery.update(
                request.symbol,
                raw_state,
                stable,
                now,
            ...
            return {
                ...
                "venue_count": 2,
                "session": (
                    request.market.session
                    or equity_session(request.market.event_time)
                ).value,
        ...
        decision = next(
            (
                ...
                if item.get("symbol") == request.symbol
            ),
            None,
        )
        ...
        provider_families = {
            ...
            if item.get("eligible") and item.get("fresh")
        }
        entitled = bool(provider_families) and all(
            self.entitlements.get(provider, {}).get(
                "non_display_computation",
                False,
            )
        ...
        divergence = (
            abs(
                request.market.mark_price / Decimal(str(reference))
                - 1
            )
            * Decimal("10000")
            if reference
        ...
        raw_state = (
            decision.get("risk_state", "HALTED")
            if decision
            else "HALTED"
        )
        stable = bool(
        ...
        return {
            "reference_price": reference,
            "reference_status": (
                decision.get("status", "INSUFFICIENT_EVIDENCE")
                if decision
                else "INSUFFICIENT_EVIDENCE"
            ),
            "confidence": (
                Decimal(str(decision.get("confidence", 0)))
                / Decimal("100")
                if decision
                else Decimal("0")
            ),
            ...
            "venue_count": (
                decision.get("independent_source_families", 0)
                if decision
                else 0
            ),
            "session": (
            ...
            "stale": (
                ...
                or any(
                    item.get("eligible") and not item.get("fresh")
                    for item in evidence
                )
            ),
            "malformed": False,
        ...
        *,
        ...
        if prior:
            if prior.request_hash != request_hash:
                raise ValueError(
                    "request_id already used for materially different order"
                )
            expires_at = datetime.fromisoformat(
                prior.passport["claims"]["decision"]["expires_at"].replace(
                    "Z",
                    "+00:00",
                )
            )
            if now > expires_at:
                raise ValueError(
                    "decision expired; submit a new request_id"
                )
        ...
        portfolio = result.pop("portfolio", {"enabled": False})
        safe_alternative = {
            "available": (
                result["permitted_notional_usd"] > 0
                and (
                    result["permitted_notional_usd"]
                    < request.intent.notional_usd
                    or result["permitted_leverage"]
                    < request.intent.requested_leverage
                )
            ),
            "max_leverage": result["permitted_leverage"],
            "max_notional_usd": result["permitted_notional_usd"],
            "message": (
                "Use the capped leverage/notional without changing Market Truth."
                if result["permitted_notional_usd"] > 0
                else (
                    "No additional risk is safe under the current "
        ...
        claims = {
            ...
            "order": {
                "intent_kind": request.intent.kind.value,
            ...
            "policy": {
                ...
                "model_version": truth.get("model_version"),
                "data_policy_version": asset_policy["data_policy"],
        ...
        return _plain(
            {
                ...
                "requested_notional_usd": claims["order"][
                    "requested_notional_usd"
                ],
                "permitted_notional_usd": claims["order"][
                    "permitted_notional_usd"
                ],
                "safe_alternative": claims["decision"].get(
                    "safe_alternative"
                ),
                "market": claims["market"],
                "account_risk": claims["account"],
                "portfolio_risk": claims.get(
                    "portfolio",
                    {"enabled": False},
                ),
    ...
    def replay(
        ...
        *,
        ...
        if policy_version == "WITHOUT_SAFETY_GATE":
            replayed = {
                "action": "ALLOW",
                "permitted_leverage": original["requested_leverage"],
                "permitted_notional_usd": original[
                    "requested_notional_usd"
                ],
            ...
            replayed = _plain(
                {
                    ...
                    if key not in {"exposure", "portfolio"}
                }
            )
        return {
            ...
            "original_permitted_notional_usd": original[
                "permitted_notional_usd"
            ],
            "replayed_permitted_notional_usd": replayed[
                "permitted_notional_usd"
            ],
            "prevented_additional_exposure_usd": max(
                0,
                original["requested_notional_usd"]
                - original["permitted_notional_usd"],
            ),
            "reason_changes": {
                "original": original["reasons"],
                "replayed": replayed["reasons"],
            },
            "deterministic_parity": (
                policy_version != "WITHOUT_SAFETY_GATE"
                and original["action"] == replayed["action"]
                and original["permitted_notional_usd"]
                == replayed["permitted_notional_usd"]
            ),
            "disclaimer": (
                "Counterfactual simulation; it is not proof that Mochatrade "
                "would have executed the ungated order."
                if policy_version == "WITHOUT_SAFETY_GATE"
                else None
            ),
        }
