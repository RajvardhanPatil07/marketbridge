...
def exposure_for(request: RiskCheckRequest, reference_price: Decimal | None) -> ExposureResult:
    account = request.account
    current = account.current_leverage
    if current is None:
        current = account.position_notional_usd / account.equity_usd
    added = Decimal("0") if request.intent.kind in {IntentKind.REDUCE, IntentKind.CLOSE} else request.intent.notional_usd
    ...
    if reference_price is not None and account.liquidation_price is not None:
        distance = abs(reference_price - account.liquidation_price) / reference_price * Decimal("100")
...
def decide(
    ...
    *,
    ...
    reference = Decimal(str(market_truth["reference_price"])) if market_truth.get("reference_price") is not None else None
    ...
    # Valid exits are invariant to market-risk evidence. Authentication and input
    # validation happen before this function and can still reject malformed calls.
    if intent.kind in {IntentKind.REDUCE, IntentKind.CLOSE}:
        return {
            "action": "ALLOW",
            "permitted_leverage": intent.requested_leverage,
            "permitted_notional_usd": min(intent.notional_usd, request.account.position_notional_usd),
    ...
    if exposure.liquidation_distance_pct is None and request.account.position_notional_usd > 0:
        action = "REVIEW"
    ...
    if exposure.classification == "ELEVATED":
        ...
        reasons.extend(["NEAR_LIQUIDATION" if exposure.liquidation_distance_pct is not None else "EXISTING_EXPOSURE"])
    elif exposure.classification == "GUARDED":
        cap = min(cap, Decimal("3"))
        reasons.append("EXISTING_EXPOSURE")

    margin_cap_notional = request.account.margin_available_usd * max(cap, Decimal("1"))
    exposure_room = max(Decimal("0"), request.account.equity_usd * cap - request.account.position_notional_usd)
    permitted_notional = min(intent.notional_usd, margin_cap_notional, exposure_room)
    permitted_notional = permitted_notional.quantize(Decimal("0.01"), rounding=ROUND_DOWN)
    permitted_leverage = min(intent.requested_leverage, cap)
    if permitted_notional <= 0:
        action = "BLOCK_NEW_RISK"
        permitted_leverage = Decimal("0")
        reasons.append("NO_RISK_CAPACITY")
    elif permitted_leverage < intent.requested_leverage or permitted_notional < intent.notional_usd:
