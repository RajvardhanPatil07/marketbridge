...
import {
  Activity, ArrowRight, CircleAlert, Fingerprint, Gauge, Radio, RefreshCw, Search,
...
const EvidenceLineage = dynamic(() => import("./evidence-lineage"), { ssr: false });

const API_BASE = (process.env.NEXT_PUBLIC_API_BASE ?? "").replace(/\/$/, "");
const money = new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 2 });
const fmt = (value: number | null | undefined, digits = 1) => value == null ? "—" : value.toFixed(digits);
const shortHash = (value: string | null) => value ? `${value.slice(0, 12)}…${value.slice(-8)}` : "Genesis";
...
export default function LiveView({ onFallback }: { onFallback: () => void }) {
  ...
  const executionFeeds = snapshot?.market_health.authenticated_multi_venue_feeds.length ?? 0;
  const freshExecutionFeeds = snapshot?.market_health.fresh_multi_venue_feeds.length ?? 0;
  const uncertainty = selectedDecision ? Math.max(0, Math.min(100, 100 - selectedDecision.confidence)) : 100;
  ...
  return <div className="live-view cmc-live">
    ...
    <section className="market-kpis">
      <div className="panel kpi"><span>Market health</span><strong className={snapshot.market_health.status === "HEALTHY" ? "positive" : "warning"}>{words(snapshot.market_health.status)}</strong><small>{connected ? `${transport} connected` : "Transport reconnecting"} · generation {snapshot.generation}</small></div>
      <div className="panel kpi"><span>Live feeds</span><strong>{healthyFeeds}/{snapshot.providers.length}</strong><small>{executionFeeds} authenticated · {freshExecutionFeeds} fresh multi-venue feed{freshExecutionFeeds === 1 ? "" : "s"}</small></div>
    ...
    {selectedDecision ? <div className="integrity-detail-grid">
      ...
      <aside className="panel risk-card"><div className={`risk-icon ${riskClass(selectedDecision.risk_state)}`}>{selectedDecision.risk_state === "NORMAL" ? <ShieldCheck/> : <ShieldOff/>}</div><span className="eyebrow">RISK RECOMMENDATION</span><h2>{selectedDecision.recommended_max_leverage}× max leverage</h2><p>Prototype advisory policy only. MarketBridge never executes a liquidation or order in this demo.</p><dl><div><dt>Confidence</dt><dd>{fmt(selectedDecision.confidence, 0)}%</dd></div><div><dt>Divergence</dt><dd>{selectedDecision.mark_divergence_bps == null ? "—" : `${fmt(selectedDecision.mark_divergence_bps)} bp`}</dd></div><div><dt>Notional multiplier</dt><dd>{Math.round(selectedDecision.max_notional_multiplier * 100)}%</dd></div><div><dt>Status</dt><dd>{words(selectedDecision.status)}</dd></div></dl><div className="uncertainty-thermostat"><div><span>Uncertainty thermostat</span><strong>{fmt(uncertainty, 0)}%</strong></div><i><b style={{ width: `${uncertainty}%` }}/></i><small>Confidence loss automatically narrows exposure and leverage.</small></div><button className="text-button" onClick={onFallback}>Stress-test this policy <ArrowRight size={14}/></button></aside>
    </div> : null}

    {selectedDecision?.ai?.enabled ? <section className="panel ai-console"><div className="panel-heading"><div><span className="eyebrow"><Activity size={12}/>AI / ML LAYER</span><h2>Learned fair value + anomaly signal</h2><p>Direct consensus always wins. The model may estimate when evidence is weak, and its anomaly score can only tighten risk after historical calibration.</p></div><span className={`ai-mode ${selectedDecision.ai.risk_signal_enforced ? "enforced" : "advisory"}`}>{selectedDecision.ai.risk_signal_enforced ? "RISK SIGNAL ACTIVE" : "ADVISORY ONLY"}</span></div><div className="ai-metrics"><div><span>AI fair value</span><strong>{selectedDecision.ai.predicted_reference == null ? "—" : money.format(selectedDecision.ai.predicted_reference)}</strong><small>{selectedDecision.ai.fair_value_used ? "Used as ESTIMATED fallback" : "Shadow prediction only"}</small></div><div><span>Predicted return</span><strong>{selectedDecision.ai.predicted_return_bps == null ? "—" : `${fmt(selectedDecision.ai.predicted_return_bps)} bp`}</strong><small>{selectedDecision.ai.fair_value_model_type ? words(selectedDecision.ai.fair_value_model_type) : "Model unavailable"}</small></div><div><span>Anomaly probability</span><strong className={(selectedDecision.ai.anomaly_probability ?? 0) >= 0.8 ? "negative" : "positive"}>{selectedDecision.ai.anomaly_probability == null ? "—" : `${fmt(selectedDecision.ai.anomaly_probability * 100, 0)}%`}</strong><small>Venue mark vs independent reference</small></div><div><span>Model provenance</span><strong>{selectedDecision.ai.model_version ?? "—"}</strong><small>{selectedDecision.ai.data_mode ? words(selectedDecision.ai.data_mode) : "No bundle"}</small></div></div>{selectedDecision.ai.top_factors.length ? <div className="ai-factors"><span>Top fair-value drivers</span>{selectedDecision.ai.top_factors.map((factor) => <div key={factor.feature}><strong>{words(factor.feature)}</strong><b>{factor.contribution_bps == null ? "—" : `${factor.contribution_bps >= 0 ? "+" : ""}${fmt(factor.contribution_bps)} bp`}</b></div>)}</div> : null}<div className="ai-boundary"><ShieldCheck size={16}/><span>The AI cannot mark an isolated price as valid, cannot loosen leverage, and never replaces the deterministic evidence firewall.</span></div></section> : null}
    ...
    {selectedDecision ? <section className="panel lineage-console"><div className="panel-heading"><div><h2>Decision lineage</h2><p>Pan and zoom from independent observations to the reference, passport and risk boundary.</p></div><span className="small-label">INTERACTIVE GRAPH</span></div><EvidenceLineage decision={selectedDecision}/></section> : null}

    {selectedDecision?.passport ? <section className="panel passport-console"><div className="panel-heading"><div><span className="eyebrow"><Fingerprint size={12}/>MARK PASSPORT</span><h2>Hash-chained decision receipt</h2><p>Sealed issued-decision claims and normalized evidence fingerprints. Any changed claim produces a different SHA-256 chain hash.</p></div><span className="passport-state"><ShieldCheck size={13}/>SEALED</span></div><div className="passport-grid"><div><span>Sequence</span><strong>#{selectedDecision.passport.sequence}</strong><small>{selectedDecision.passport.scope}</small></div><div><span>Chain hash</span><strong>{shortHash(selectedDecision.passport.chain_hash)}</strong><small>SHA-256</small></div><div><span>Previous receipt</span><strong>{shortHash(selectedDecision.passport.previous_hash)}</strong><small>{selectedDecision.passport.previous_hash ? "Linked predecessor" : "Chain origin"}</small></div><div><span>Attested evidence</span><strong>{selectedDecision.passport.claims.evidence.length}</strong><small>{selectedDecision.passport.claims.policy_version}</small></div></div><div className="passport-boundary"><Fingerprint size={15}/><span>This passport attests the decision as issued. Live freshness can later tighten the displayed risk state without rewriting its historical receipt.</span></div></section> : null}
