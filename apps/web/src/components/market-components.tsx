...
export function Sparkline({ asset }: { asset: MarketAsset }) {
  const values = asset.points.slice(-32).map((point) => point.value);
  ...
  return <svg className={`market-sparkline ${values.at(-1)! < values[0] ? "negative" : "positive"}`} viewBox="0 0 96 28" role="img" aria-label={`${asset.symbol} recent price trend`}><polyline points={points} fill="none" stroke="currentColor" strokeWidth="1.5" vectorEffect="non-scaling-stroke"/></svg>;
}
